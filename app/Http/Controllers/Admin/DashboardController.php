    ...
    public function index()
    {
        ...
        $chartRaw = LoginLogs::where('user_type', 'mahasiswa')
            ->when($admin->role !== 'God', function ($q) use ($admin) {
                $q->whereHas('mahasiswa', function ($m) use ($admin) {
                    $m->where('prodi_id', $admin->program_studi_id);
                });
            })
        ...
        $aduanChart = Pengaduan::when($admin->role !== 'God', function ($q) use ($admin) {
            $q->where('program_studi_id', $admin->program_studi_id);
        })
        ...
        $labels = $chartRaw->pluck('date')->map(fn($d) => \Carbon\Carbon::parse($d)->format('d/m'));
        ...
        $totalMahasiswa = Mahasiswa::when($admin->role !== 'God', function ($q) use ($admin) {
            $q->where('prodi_id', $admin->program_studi_id);
        })
            ->count();
        $aduanMahasiswa = Pengaduan::when($admin->role !== 'God', function ($q) use ($admin) {
            $q->where('program_studi_id', $admin->program_studi_id);
        })
            ->count();
